...
const fetchUser = async () =>{
    ...
    try{
        const res = await axios.get(`${staff}/eva_score/user/${id_eva}`,{headers:{Authorization:`Bearer ${token}`}})
        user.value = res.data
...
const fetchTopic = async () =>{
    const token = localStorage.getItem('token')
    try{
        const res = await axios.get(`${staff}/eva_score/topic/${id_eva}`,{headers:{Authorization:`Bearer ${token}`}})
