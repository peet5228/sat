                ...
                <v-card>
                    ...
                    <v-card-text>
                        <v-form @submit.prevent="saveMember">
                            <v-row>
                                <v-col cols="12" md="6">
                                    <v-select label="หัวข้อการประเมิน" :items="topic.map(p => ({title:p.name_topic , value: p.id_topic}))" v-model="form.id_topic" :error-messages="error.id_topic" />
                                </v-col>
                                <v-col cols="12" md="6">
                                    <v-text-field label="ชื่อ" v-model="form.name_indicate" :error-messages="error.name_indicate"></v-text-field>
                                </v-col>
                                <v-col cols="12" md="12">
                                ...
                                <v-col cols="12" md="6">
                                    <v-select label="ลักษณะตัวเลือกคะแนน" :items="[{title:'มี',value:'y'},{title:'ไม่มี',value:'n'}]" v-model="form.check_indicate" :error-messages="error.point_indicate" />
                                </v-col>
                                <v-row>
                                    <v-col cols="12" md="6">
                                        <v-btn class="text-white" type="submit" block color="blue">{{ form.id_indicate ? 'อัปเดต' : 'บันทึก' }}</v-btn>
                                    </v-col>
                                    <v-col cols="12" md="6">
                        ...
                        <v-table>
                            ...
                            <tbody>
                                <tr v-for="(items,index) in result" :keys="items.id_indicate">
                                    ...
                                    <td class="text-center border">
                                        ...
                                        <v-btn class="text-white" color="error" @click="del(items.id_indicate)" size="small">ลบ</v-btn>
                                    </td>
                                </tr>
...
function validateForm(){
    ...
    if(!f.name_indicate)error.value.name_indicate='กรุุณาชื่อตัวชี้วัด!'
