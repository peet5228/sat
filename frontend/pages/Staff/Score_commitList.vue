                            ...
                            <tbody>
                                <tr v-for="(items,index) in result" :keys="items.id_eva">
                                    ...
                                    <td class="text-center border">
                                        <v-btn class="text-white font-weight-bold" size="small" :color="bg(items.status_eva)">{{ items.status_eva === 1 ? 'ยังไม่ได้ประเมิน' : items.status_eva === 2 ? 'รอกรรมการประเมิน' : 'ประเมินเสร็จสิ้น' }}</v-btn>
                                    </td>
                                    <td class="text-center border">
...
const go = (id_eva:number) => {
    navigateTo({path: `Commit_score-${id_eva}`})
}
